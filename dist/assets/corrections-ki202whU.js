import{j as fe,g as ge,r as ye,i as Se,a as ve,e as Ae,f as ne,b as I,c as _,s as P,d as D,n as R,h as we,k as Ne,D as j,l as H}from"./utils-Bld6jg1h.js";import{G as re}from"./gemini-C7Dr6FyN.js";const w="gemini-2.5-flash",N="gemini-2.5-flash-lite-preview-06-17",C="gemma-3-27b-it",ae="gemma-3n-e4b-it",E=.8,Ie=10,_e=15,Ee=30,be=30,Te=["Fantasy & Myth","Science Fiction & Future","Horror & Dark Mystery","Action & Wasteland","Testing"],S=3,Ze=3,et=50,tt="Eliot the Cougar",nt="1.4.0 (Ink and Quill)",rt="7",at="whispersInTheDark_gameState",ot="whispersInTheDark_debugPacket",it="whispersInTheDark_debugLore",oe="whispersInTheDark_geminiApiKey",st=30,ct=5,lt=Te.filter(e=>e!=="Testing"),dt="Male",ie="player",pt="player_journal",ut=6,mt=70,ht=5,ft=5,gt=10,yt=10,Le=4,$e=10,U=["single-use","multi-use","equipment","container","key","weapon","ammunition","vehicle","immovable","status effect","page","book","picture","map"],Y=U.map(e=>e).join(", "),Ce=["create","change","move","destroy","addDetails"],Pe=Ce.map(e=>e).join(", "),M=["default","junk"],De=["stashed"],K=["printed","handwritten","typed","digital"],se=["faded","smudged","torn","glitching","encrypted","foreign","gothic","runic","recovered"],F=[...K,...se],Oe=[...M,...De,...F],St=M.map(e=>e).join(", "),Me=K.map(e=>e).join(", "),vt=se.map(e=>e).join(", "),Re=["inspect","use","drop","discard","enter","park","read","write"],ke=Re.map(e=>e).join(", "),x=["distant","nearby","companion","unknown"],Ue=x.map(e=>e).join(", "),b={storyteller:{text:"Dungeon Master thinks...",icon:"░░"},map:{text:"Cartographer draws the map...",icon:"░░"},correction:{text:"Dungeon Master is fixing mistakes...",icon:"▓"},inventory:{text:"Dungeon Master handles items...",icon:"░░"},dialogue_turn:{text:"Conversation continues...",icon:"░░"},dialogue_summary:{text:"Dialogue concludes...",icon:"░░"},dialogue_memory_creation:{text:"Memories form...",icon:"░░"},dialogue_conclusion_summary:{text:"Returning to the world...",icon:"░░"},initial_load:{text:"Loading...",icon:"░░"},reality_shift_load:{text:"Reality shifts...",icon:"░░"},visualize:{text:"Visualizing the scene...",icon:"░░"},page:{text:"Reading...",icon:"░░"},journal:{text:"Writing...",icon:"░░"},loremaster_collect:{text:"Loremaster picks facts...",icon:"░"},loremaster_extract:{text:"Loremaster extracts new lore...",icon:"░"},loremaster_write:{text:"Loremaster writes down lore...",icon:"░"},loremaster_refine:{text:"Loremaster refines lore...",icon:"░"},book:{text:"Reading...",icon:"░░"}},ce=["undiscovered","discovered","rumored","quest_target","blocked"],At=ce.map(e=>e).join(", "),le=["region","location","settlement","district","exterior","interior","room","feature"],wt=le.map(e=>e).join(", "),J=le.reduce((e,t,n)=>(e[t]=n,e),{}),de=["path","road","sea route","door","teleporter","secret_passage","river_crossing","temporary_bridge","boarding_hook","shortcut"],Nt=de.map(e=>e).join(", "),pe=["open","accessible","closed","locked","blocked","hidden","rumored","one_way","collapsed","removed","active","inactive"],It=pe.map(e=>e).join(", "),_t=["collapsed","hidden","removed"],Et=20,B=1e3,W=750,bt=`${String(-B/2)} ${String(-W/2)} ${String(B)} ${String(W)}`,Tt=8,Lt=4,$t=10,Ct=1.1,Pt=2,Dt=.3,Ot=5,Mt=4,Rt=8,kt=10,G="a short creative description of the location, <300 chars",xe="a short creative description, focusing of travel conditions of the path",V="alternative names, partial names, shorthands. Avoid generic common terms.",je=`Valid item "type" values are: ${Y}.
...
`,Ut=`Generate inventory hints using these fields:
...
- Make sure that 'book' type items have between ${String(Le)} and ${String($e)} chapters.
- Make sure 'page', 'book', 'map' and 'picture' type items have one of the required tags: ${Me}.
...
IMPORTANT: NEVER add ${ke} known uses - there are dedicated buttons for those in the game.
...
- feature: Notable sub-location or landmark within any other node.`,Ve=`Map Edge Types:
...
- shortcut: Any special connection that bypasses hierarchy rules.`,Ye=`Map Node Hierarchy:
...
- Only "feature" nodes can be connected to each other with edges.`,xt=`- You MUST provide "localTime", "localEnvironment", "localPlace" in the response.
...
`;var q={};let L=null,k=!1;typeof localStorage<"u"&&(L=localStorage.getItem(oe));L||(typeof window<"u"&&window.GEMINI_API_KEY?(L=window.GEMINI_API_KEY,k=!0):(L=q.GEMINI_API_KEY??q.API_KEY??null,L&&(k=!0)));L||console.error("Gemini API key is not set. Gemini services will be unavailable.");let ue=L?new re({apiKey:L}):null;const A=()=>!!L,jt=()=>k,Gt=()=>L,Vt=e=>{L=e,k=!1,typeof localStorage<"u"&&localStorage.setItem(oe,e),ue=new re({apiKey:e})};A()||console.error("Gemini API key is not set. The application will not be able to connect to the Gemini API.");const X=ue,Ke={[w]:["thinking","system","schema"],[N]:["thinking","system","schema"],[C]:[],[ae]:[]},T=async e=>{var o,c,s;if(!A()||!X)return Promise.reject(new Error("API Key not configured."));const t={[w]:Ie,[N]:_e,[C]:Ee,[ae]:be};let n=null;for(const a of e.modelNames){const[r,l]=Array.isArray(a)?a:[a,Ke[a]??[]],d=l.includes("system"),p=l.includes("thinking"),f=l.includes("schema");let i=e.systemInstruction??"";if(!f&&e.jsonSchema){const m=fe(e.jsonSchema);i=i?`${i}

${m}`:m}const h=d?e.prompt:`${i?i+`

`:""}${e.prompt}`,u={};if(e.temperature!==void 0&&(u.temperature=e.temperature),e.responseMimeType&&f&&(u.responseMimeType=e.responseMimeType),p&&(e.thinkingBudget!==void 0||e.includeThoughts)){const m={};e.thinkingBudget!==void 0&&(m.thinkingBudget=e.thinkingBudget),e.includeThoughts&&(m.includeThoughts=!0),u.thinkingConfig=m}d&&i&&(u.systemInstruction=i),f&&e.jsonSchema&&(u.responseJsonSchema=e.jsonSchema);for(let m=1;m<=S;){const y=ge(r,t[r]??1);if(y>0||m>1){const g=5e3+y;await new Promise(v=>setTimeout(v,g))}try{const g=await X.models.generateContent({model:r,contents:h,config:u});return ye(r),e.label&&console.log(`[${e.label}] ${r} tokens: total ${String(((o=g.usageMetadata)==null?void 0:o.totalTokenCount)??"N/A")}, prompt ${String(((c=g.usageMetadata)==null?void 0:c.promptTokenCount)??"N/A")}, thoughts ${String(((s=g.usageMetadata)==null?void 0:s.thoughtsTokenCount)??"N/A")}`),e.debugLog&&e.debugLog.push({prompt:e.prompt,systemInstruction:i,jsonSchema:e.jsonSchema,modelUsed:r,responseText:g.text??"",promptUsed:h}),{response:g,modelUsed:r,systemInstructionUsed:i,jsonSchemaUsed:f?e.jsonSchema:void 0,promptUsed:h}}catch(g){if(e.debugLog&&e.debugLog.push({prompt:e.prompt,systemInstruction:i,jsonSchema:e.jsonSchema,modelUsed:r,responseText:`ERROR: ${g instanceof Error?g.message:String(g)}`,promptUsed:h}),n=g,!Se(g)&&!ve(g))throw g;const v=Ae(g),O=g instanceof Error?g.message:String(g),$=v!==null?String(v):O;console.warn(`dispatchAIRequest: Model ${r} failed with ${$}. Retry ${String(m)}/${String(S)}`),m+=1}}console.warn(`dispatchAIRequest: Model ${r} exhausted retries. Falling back if another model is available.`)}throw n instanceof Error?n:new Error(String(n))},Yt=async(e,t,n,o,c)=>{if(!A())return console.error(`fetchCorrectedNPCDetails_Service: API Key not configured. Cannot fetch details for "${e}".`),null;const s=c.length>0?"Known map locations in this theme: "+ne(c,!0):"No specific map locations are currently known for this theme.",a=`
...
`,r="You generate detailed JSON objects for new NPCs based on narrative context. Provide description, aliases, presenceStatus, lastKnownLocation, and preciseLocation. Adhere strictly to the JSON format and field requirements. Derive all information strictly from the provided context.";return I(async l=>{try{_(b.correction.icon);const{response:d}=await T({modelNames:[N,w],prompt:a,systemInstruction:r,responseMimeType:"application/json",temperature:E,label:"Corrections"}),p=P(D(d.text??""));if(p&&typeof p.description=="string"&&p.description.trim()!==""&&Array.isArray(p.aliases)&&p.aliases.every(f=>typeof f=="string")&&typeof p.presenceStatus=="string"&&x.includes(p.presenceStatus)&&(p.lastKnownLocation===null||typeof p.lastKnownLocation=="string")&&(p.preciseLocation===null||typeof p.preciseLocation=="string")&&!((p.presenceStatus==="nearby"||p.presenceStatus==="companion")&&(p.preciseLocation===null||p.preciseLocation===""))&&!((p.presenceStatus==="distant"||p.presenceStatus==="unknown")&&p.preciseLocation!==null))return{result:p};console.warn(`fetchCorrectedNPCDetails_Service (Attempt ${String(l+1)}/${String(S+1)}): Corrected details for "${e}" invalid or incomplete. Response:`,p)}catch(d){throw console.error(`fetchCorrectedNPCDetails_Service error (Attempt ${String(l+1)}/${String(S+1)}):`,d),d}return{result:null}})},z=new Set(K),Fe=["page","book","map","picture"],Q=new Set(Fe);function He(e,t){const n=`${e} ${t}`.toLowerCase();return/(handwritten|scribbled|ink|pen|quill)/.test(n)?"handwritten":/(typewriter|typed)/.test(n)?"typed":/(digital|screen|display|tablet|monitor|terminal)/.test(n)?"digital":"printed"}function me(e){if(!e||typeof e!="object")return!1;const t=e;return!(typeof t.actionName!="string"||t.actionName.trim()===""||typeof t.promptEffect!="string"||t.promptEffect.trim()===""||t.appliesWhenActive!==void 0&&typeof t.appliesWhenActive!="boolean"||t.appliesWhenInactive!==void 0&&typeof t.appliesWhenInactive!="boolean"||typeof t.description!="string"||t.description.trim()==="")}function Z(e,t){if(!e||typeof e!="object")return!1;const n=e;if(typeof n.type=="string"){const c=R(n.type);c&&(n.type=c)}if(typeof n.name!="string"||n.name.trim()==="")return console.warn("isValidItem: 'name' is missing or invalid.",e),!1;if(t==="create"||!t){if(typeof n.type!="string"||!U.includes(n.type))return console.warn(`isValidItem (context: ${t??"default"}): 'type' is missing or invalid.`,e),!1;if(typeof n.description!="string"||n.description.trim()==="")return console.warn(`isValidItem (context: ${t??"default"}): 'description' is missing or invalid.`,e),!1;if(typeof n.holderId!="string"||n.holderId.trim()==="")return console.warn(`isValidItem (context: ${t??"default"}): 'holderId' is missing or invalid.`,e),!1}if(t==="change"&&n.newName!=null&&(typeof n.newName!="string"||n.newName.trim()===""))return console.warn("isValidItem (context: change, with newName): 'newName' is invalid.",e),!1;if(n.type!==void 0){const c=R(n.type);if(!c)return console.warn("isValidItem: 'type' is present but invalid.",e),!1;n.type=c}if(n.description!==void 0&&(typeof n.description!="string"||n.description.trim()==="")&&(t==="create"||t==="change"&&n.newName)&&n.description.trim()==="")return console.warn("isValidItem: 'description' is present but empty, which is invalid for a create or transformation.",e),!1;if(n.activeDescription!==void 0&&typeof n.activeDescription!="string")return console.warn("isValidItem: 'activeDescription' is present but invalid.",e),!1;if(n.isActive!==void 0&&typeof n.isActive!="boolean")return console.warn("isValidItem: 'isActive' is present but invalid.",e),!1;if(n.stashed!==void 0&&typeof n.stashed!="boolean")return console.warn("isValidItem: 'stashed' is present but invalid.",e),!1;if(n.tags!==void 0){if(!Array.isArray(n.tags)||!n.tags.every(a=>typeof a=="string"))return console.warn("isValidItem: 'tags' is present but invalid.",e),!1;const c=we(n.tags);c?n.tags=c:n.tags=n.tags.filter(a=>Oe.includes(a));const s=Q.has(n.type??"")?[...M,...F]:M;n.tags=n.tags.filter(a=>s.includes(a))}if(Q.has(n.type??"")){n.tags=n.tags??[];const c=n.tags.filter(s=>z.has(s));if(c.length===0){const s=He(n.name,n.description??"");n.tags.unshift(s)}else if(c.length>1){const[s]=c;n.tags=[s,...n.tags.filter(a=>!z.has(a))]}}if(n.holderId!==void 0&&(typeof n.holderId!="string"||n.holderId.trim()===""))return console.warn("isValidItem: 'holderId' is present but invalid.",e),!1;const o=c=>Array.isArray(c)&&c.every(s=>s&&typeof s=="object"&&typeof s.heading=="string"&&typeof s.description=="string"&&typeof s.contentLength=="number"&&(s.imageData===void 0||typeof s.imageData=="string"));if(n.type==="page"||n.type==="book"||n.type==="map"||n.type==="picture"){if(n.chapters!==void 0){if(!o(n.chapters))return console.warn("isValidItem: 'chapters' is present but invalid.",e),!1;(n.type==="page"||n.type==="map"||n.type==="picture")&&n.chapters.length>1&&(n.chapters=[n.chapters[0]])}else{const c=typeof n.contentLength=="number"?n.contentLength:30;n.chapters=[{heading:n.name,description:n.description??"",contentLength:c,actualContent:typeof n.actualContent=="string"?n.actualContent:void 0,visibleContent:typeof n.visibleContent=="string"?n.visibleContent:void 0}]}delete n.contentLength,delete n.actualContent,delete n.visibleContent}else if(n.chapters!==void 0&&!o(n.chapters))return console.warn("isValidItem: 'chapters' is present but invalid for non-book/page item.",e),!1;return n.contentLength!==void 0&&typeof n.contentLength!="number"?(console.warn("isValidItem: 'contentLength' is present but invalid.",e),!1):n.actualContent!==void 0&&typeof n.actualContent!="string"?(console.warn("isValidItem: 'actualContent' is present but invalid.",e),!1):n.visibleContent!==void 0&&typeof n.visibleContent!="string"?(console.warn("isValidItem: 'visibleContent' is present but invalid.",e),!1):n.knownUses!==void 0&&!(Array.isArray(n.knownUses)&&n.knownUses.every(me))?(console.warn("isValidItem: 'knownUses' is present but invalid.",e),!1):!0}function ee(e){if(!e||typeof e!="object")return!1;const t=e;return typeof t.id=="string"&&t.id.trim()!==""&&typeof t.name=="string"&&t.name.trim()!==""}function he(e){if(!e||typeof e!="object")return!1;const t=e;if(typeof t.id!="string"||t.id.trim()===""||typeof t.name!="string"||t.name.trim()===""||typeof t.type!="string"||!U.includes(t.type))return!1;const n=["page","book","map","picture"].includes(t.type);if(t.knownUses===void 0&&t.tags===void 0&&t.chapters===void 0||t.knownUses!==void 0&&!(Array.isArray(t.knownUses)&&t.knownUses.every(me)))return!1;const o=n?[...M,...F]:M,c=Array.isArray(t.tags)&&t.tags.every(a=>o.includes(a)),s=Array.isArray(t.chapters)&&t.chapters.every(a=>{const r=a;return typeof r.heading=="string"&&typeof r.description=="string"&&typeof r.contentLength=="number"});if(n){if(!c||!s)return!1}else if(t.tags!==void 0&&!c||t.chapters!==void 0&&!s)return!1;return!0}function Kt(e){if(!e||typeof e!="object")return!1;const t=e;if(typeof t.type=="string"){const n=R(t.type);n&&(t.type=n)}return typeof t.name=="string"&&t.name.trim()!==""&&typeof t.description=="string"&&t.description.trim()!==""&&typeof t.type=="string"&&U.includes(t.type)}function Ft(e){if(!e||typeof e!="object")return!1;const t=e;return typeof t.name!="string"||t.name.trim()===""||t.newDescription!==void 0&&typeof t.newDescription!="string"||t.newAliases!==void 0&&!(Array.isArray(t.newAliases)&&t.newAliases.every(n=>typeof n=="string"))||t.addAlias!==void 0&&typeof t.addAlias!="string"||t.newPresenceStatus!==void 0&&!x.includes(t.newPresenceStatus)||t.newLastKnownLocation!==void 0&&t.newLastKnownLocation!=null&&typeof t.newLastKnownLocation!="string"||t.newPreciseLocation!==void 0&&t.newPreciseLocation!=null&&typeof t.newPreciseLocation!="string"?!1:((t.newPresenceStatus==="nearby"||t.newPresenceStatus==="companion")&&t.newPreciseLocation,(t.newPresenceStatus==="distant"||t.newPresenceStatus==="unknown")&&t.newPreciseLocation!=null,!0)}function Ht(e){if(!e||typeof e!="object")return!1;const t=e;return typeof t.name!="string"||t.name.trim()===""||typeof t.description!="string"||t.description.trim()===""||t.aliases!==void 0&&!(Array.isArray(t.aliases)&&t.aliases.every(n=>typeof n=="string"))||t.presenceStatus!==void 0&&!x.includes(t.presenceStatus)||t.lastKnownLocation!==void 0&&t.lastKnownLocation!=null&&typeof t.lastKnownLocation!="string"||t.preciseLocation!==void 0&&t.preciseLocation!=null&&typeof t.preciseLocation!="string"?!1:((t.presenceStatus==="nearby"||t.presenceStatus==="companion")&&t.preciseLocation,(t.presenceStatus==="distant"||t.presenceStatus==="unknown")&&t.preciseLocation!=null,!0)}function Je(e){if(!e||typeof e!="object")return console.warn("isDialogueSetupPayloadStructurallyValid: dialogueSetup is missing or not an object."),!1;const t=e;if(!Array.isArray(t.participants)||t.participants.length===0||!t.participants.every(o=>typeof o=="string"&&o.trim()!==""))return console.warn("isDialogueSetupPayloadStructurallyValid: dialogueSetup.participants is invalid.",t.participants),!1;const n=t.participants;return!Array.isArray(t.initialNpcResponses)||t.initialNpcResponses.length===0||!t.initialNpcResponses.every(o=>o&&typeof o.speaker=="string"&&o.speaker.trim()!==""&&n.includes(o.speaker)&&typeof o.line=="string"&&o.line.trim()!=="")?(console.warn("isDialogueSetupPayloadStructurallyValid: dialogueSetup.initialNpcResponses is invalid.",t.initialNpcResponses),!1):!Array.isArray(t.initialPlayerOptions)||t.initialPlayerOptions.length<4||!t.initialPlayerOptions.every(o=>typeof o=="string"&&o.trim()!=="")?(console.warn("isDialogueSetupPayloadStructurallyValid: dialogueSetup.initialPlayerOptions is invalid.",t.initialPlayerOptions),!1):!0}const Jt=async(e,t,n,o)=>{if(!A())return console.error("fetchAdditionalBookChapters_Service: API Key not configured."),null;if(o<=0)return[];const c=n.map(r=>`- ${r}`).join(`
...
Task: Provide ${String(o)} additional chapter objects as JSON array. Each object must have "heading", "description", and "contentLength" (50-200).`,a=`Return ONLY the JSON array of ${String(o)} chapter objects.`;return I(async r=>{try{_(b.correction.icon);const{response:l}=await T({modelNames:[C],prompt:s,systemInstruction:a,responseMimeType:"application/json",temperature:E,label:"Corrections"}),d=l.text??"",p=P(D(d));if(Array.isArray(p)&&p.every(f=>typeof f.heading=="string"))return{result:p};console.warn(`fetchAdditionalBookChapters_Service (Attempt ${String(r+1)}/${String(S+1)}): invalid response`,p)}catch(l){throw console.error(`fetchAdditionalBookChapters_Service error (Attempt ${String(r+1)}/${String(S+1)}):`,l),l}return{result:null}})},Bt=async(e,t,n,o)=>{if(!A())return console.error("fetchCorrectedAddDetailsPayload_Service: API Key not configured."),null;const c=`You are an AI assistant fixing a malformed addDetails JSON object for a text adventure game.
...
Task: Provide ONLY the corrected JSON object with fields { "id": string, "name": string, "type": (${Y}), "knownUses"?, "tags"?, "chapters"? }.`,s="Return only the corrected addDetails JSON object.";return I(async a=>{try{_(b.correction.icon);const{response:r}=await T({modelNames:[N,w],prompt:c,systemInstruction:s,responseMimeType:"application/json",temperature:E,label:"Corrections"}),l=r.text??"",d=P(D(l));if(d&&he(d))return{result:d};console.warn(`fetchCorrectedAddDetailsPayload_Service (Attempt ${String(a+1)}/${String(S+1)}): invalid response`,d)}catch(r){throw console.error(`fetchCorrectedAddDetailsPayload_Service error (Attempt ${String(a+1)}/${String(S+1)}):`,r),r}return{result:null}})},Wt=async(e,t,n,o)=>{if(!A())return console.error("fetchFullPlaceDetailsForNewMapNode_Service: API Key not configured."),null;const c=`You are an AI assistant that generates detailed information for a new game map location (a main MapNode) that has just been added to the game map. The Map AI should have provided these details, but this is a fallback.
...
Respond ONLY with the single, complete JSON object.`,s=`Generate detailed JSON for a new game map location. The 'name' field in the output is predetermined and MUST match the input. Focus on creating ${G} and aliases (${V}, array, can be empty). Adhere strictly to the JSON format.`;return I(async a=>{try{_(b.correction.icon);const{response:r}=await T({modelNames:[N,w],prompt:c,systemInstruction:s,responseMimeType:"application/json",temperature:E,label:"Corrections"}),l=P(D(r.text??""));if(l&&typeof l.name=="string"&&l.name===e&&typeof l.description=="string"&&l.description.trim()!==""&&Array.isArray(l.aliases)&&l.aliases.every(d=>typeof d=="string"))return{result:l};console.warn(`fetchFullPlaceDetailsForNewMapNode_Service (Attempt ${String(a+1)}/$${String(S+1)}): Corrected map location payload invalid or name mismatch for "${e}". Response:`,l)}catch(r){throw console.error(`fetchFullPlaceDetailsForNewMapNode_Service error (Attempt ${String(a+1)}/$${String(S+1)}):`,r),r}return{result:null}})},qt=async(e,t,n)=>{if(!A())return console.error("fetchLikelyParentNode_Service: API Key not configured."),null;const o=t.currentMapNodeId&&t.themeNodes.find(i=>i.id===t.currentMapNodeId),c=new Map;t.themeNodes.forEach(i=>c.set(i.id,i));const s=new Map;t.themeNodes.forEach(i=>s.set(i.id,new Set)),t.themeEdges.forEach(i=>{s.has(i.sourceNodeId)||s.set(i.sourceNodeId,new Set),s.has(i.targetNodeId)||s.set(i.targetNodeId,new Set);const h=s.get(i.sourceNodeId);h&&h.add(i.targetNodeId);const u=s.get(i.targetNodeId);u&&u.add(i.sourceNodeId)});const a=e.nodeType??"feature",r=J[a],l=t.themeNodes.filter(i=>J[i.data.nodeType]<r).map(i=>`- ${i.id} ("${i.placeName}")`).join(`
...
Respond ONLY with the name or id of the best parent node, or "Universe" if none.`,f='Choose the most logical parent node name or id for the provided Map Node. If none is suitable use "Universe".';return I(async i=>{var h;try{_(b.correction.icon);const{response:u}=await T({modelNames:[C,N,w],prompt:p,systemInstruction:f,temperature:E,label:"Corrections",debugLog:n}),m=(h=u.text)==null?void 0:h.trim();if(m)return{result:m.trim()}}catch(u){throw console.error(`fetchLikelyParentNode_Service error (Attempt ${String(i+1)}/$${String(S+1)}):`,u),u}return{result:null}})},Xt=async(e,t,n)=>{if(!A())return console.error("fetchCorrectedNodeIdentifier_Service: API Key not configured."),null;const o=t.themeNodes.map(a=>`- ${a.id} ("${a.placeName}")`).join(`
`),c=`A different AI referred to a map location using an incorrect identifier: "${e}".
Known map nodes in the current theme:
${o}
Choose the most likely intended node ID from the list above. Respond with an empty string if none match.`,s="Respond ONLY with a single node ID from the list or an empty string.";return I(async a=>{var r;try{_(b.correction.icon);const{response:l}=await T({modelNames:[C,N,w],prompt:c,systemInstruction:s,temperature:E,label:"Corrections",debugLog:n}),d=(r=l.text)==null?void 0:r.trim();if(d){const p=d.trim(),f=t.themeNodes.find(h=>h.id===p);if(f)return{result:f.id};const i=t.themeNodes.find(h=>h.placeName===p);if(i)return{result:i.id}}}catch(l){throw console.error(`fetchCorrectedNodeIdentifier_Service error (Attempt ${String(a+1)}/$${String(S+1)}):`,l),l}return{result:null}})},Be={type:"object",properties:{observations:{type:"string",minLength:1500,description:"Contextually relevant observations about the chains and map graph."},rationale:{type:"string",minLength:1e3,description:"Explain the reasoning behind your chain fixes and refinement suggestions."},nodesToAdd:{type:"array",description:"List of nodes to add to the map.",minItems:1,items:{type:"object",properties:{placeName:{type:"string",description:"A contextually relevant location name, based on Theme and Scene Description"},data:{type:"object",properties:{description:{type:"string",minLength:30,description:G},aliases:{type:"array",description:V,minItems:2,items:{type:"string"}},status:{enum:ce},nodeType:{enum:["feature"]},parentNodeId:{type:"string",description:"Name of the Parent Node this feature belongs to, or 'Universe' (keyword for root node) if it has no parent"}},required:["description","aliases","status","nodeType","parentNodeId"],additionalProperties:!1}},required:["placeName","data"],additionalProperties:!1}},edgesToAdd:{type:"array",items:{type:"object",properties:{sourcePlaceName:{type:"string",description:"Name of the source feature node. MUST be a feature type node."},targetPlaceName:{type:"string",description:"Name of the target feature node. MUST be a feature type node."},data:{type:"object",properties:{type:{enum:de},status:{enum:pe},description:{type:"string",minLength:30,description:xe}},required:["type","status","description"],additionalProperties:!1}},required:["sourcePlaceName","targetPlaceName","data"],additionalProperties:!1}}},required:["observations","rationale","nodesToAdd","edgesToAdd"],additionalProperties:!1},zt=async(e,t)=>{if(!A()||e.length===0)return{payload:null,debugInfo:null};const o=(()=>{const r=new Map,l=new Map,d=[];e.forEach((i,h)=>{const u=new Set,m=[];[...i.sourceChain,...i.targetChain.slice().reverse()].forEach(y=>{y.data.nodeType!=="feature"&&!u.has(y.id)&&(m.push(y),u.add(y.id),r.set(y.id,y))}),m.length===0&&(u.has(i.originalSource.id)||(m.push(i.originalSource),u.add(i.originalSource.id),r.set(i.originalSource.id,i.originalSource)),u.has(i.originalTarget.id)||(m.push(i.originalTarget),u.add(i.originalTarget.id),r.set(i.originalTarget.id,i.originalTarget)));for(let y=0;y<m.length-1;y++){const g=m[y],v=m[y+1],O=g.id<v.id?`${g.id}|${v.id}`:`${v.id}|${g.id}`;l.has(O)||l.set(O,{source:g,target:v,data:i.edgeData})}d.push(`Chain ${String(h+1)}: ${m.map(y=>`"${y.placeName}"`).join(" -> ")}`)});const p=Array.from(r.values()).map((i,h)=>{const u=t.themeNodes.filter(m=>m.data.parentNodeId===i.id&&m.data.nodeType==="feature").map(m=>` - "${m.placeName}" (${m.data.nodeType}, ${m.data.status}, ${m.data.description})`).join(`
...
${Ve}
${Ye}
`;return await I(async r=>{var l,d,p;try{console.log(`fetchConnectorChains_Service (Attempt ${String(r+1)}/${String(S+1)})`),_(b.correction.icon);const{response:f}=await T({modelNames:[N,w],prompt:c,systemInstruction:s,thinkingBudget:2048,includeThoughts:!0,responseMimeType:"application/json",jsonSchema:Be,temperature:E,label:"Corrections"}),h=(((p=(d=(l=f.candidates)==null?void 0:l[0])==null?void 0:d.content)==null?void 0:p.parts)??[]).filter(v=>v.thought===!0&&typeof v.text=="string").map(v=>v.text),u={prompt:c,rawResponse:f.text??"",parsedPayload:void 0,validationError:void 0,observations:void 0,rationale:void 0,thoughts:h.length>0?h:void 0},m=D(f.text??""),y=P(m);if(!y)return u.validationError="Failed to parse JSON",{result:{payload:null,debugInfo:u}};let g=null;return Array.isArray(y)?g=y.reduce((v,O)=>{if(O&&typeof O=="object"){const $=O;Array.isArray($.nodesToAdd)&&(v.nodesToAdd=[...v.nodesToAdd??[],...$.nodesToAdd]),Array.isArray($.edgesToAdd)&&(v.edgesToAdd=[...v.edgesToAdd??[],...$.edgesToAdd]),$.observations&&!v.observations&&(v.observations=$.observations),$.rationale&&!v.rationale&&(v.rationale=$.rationale)}return v},{}):typeof y=="object"&&(g=y),u.parsedPayload=g??void 0,g&&(g.observations&&!u.observations&&(u.observations=g.observations),g.rationale&&!u.rationale&&(u.rationale=g.rationale)),g&&(g.nodesToAdd||g.edgesToAdd)?{result:{payload:g,debugInfo:u}}:(u.validationError="Parsed JSON missing nodesToAdd or edgesToAdd",{result:{payload:null,debugInfo:u}})}catch(f){throw console.error(`fetchConnectorChains_Service error (Attempt ${String(r+1)}/${String(S+1)}):`,f),f}})??{payload:null,debugInfo:null}},Qt=async(e,t,n,o)=>{if(!A())return console.error("decideFeatureHierarchyUpgrade_Service: API Key not configured."),null;const c=`A feature node has acquired a child which violates the map hierarchy rules.
Parent Feature: "${e.placeName}" (Desc: "${e.data.description}")
Child Node: "${t.placeName}" (Type: ${t.data.nodeType})
Choose the best fix: "convert_child" to make the child a sibling, or "upgrade_parent" to upgrade the parent to a higher-level node.`,s="Respond only with convert_child or upgrade_parent.";return I(async a=>{var r;try{_(b.correction.icon);const{response:l}=await T({modelNames:[C,N,w],prompt:c,systemInstruction:s,temperature:E,label:"Corrections",debugLog:o}),d=((r=l.text)==null?void 0:r.trim())??null;if(d){const p=d.trim().toLowerCase();if(p.includes("upgrade"))return{result:"upgrade_parent"};if(p.includes("convert")||p.includes("sibling"))return{result:"convert_child"}}}catch(l){throw console.error(`decideFeatureHierarchyUpgrade_Service error (Attempt ${String(a+1)}/$${String(S+1)}):`,l),l}return{result:null}})},Zt=async(e,t)=>{if(!A())return console.error("chooseHierarchyResolution_Service: API Key not configured."),null;const n=e.options.map((s,a)=>`${String(a+1)}. ${s}`).join(`
...
${n}
Respond ONLY with the option number.`,c="Answer with the single number of the best option.";return I(async s=>{var a;try{_(b.correction.icon);const{response:r}=await T({modelNames:[C,N,w],prompt:o,systemInstruction:c,temperature:E,label:"Corrections",debugLog:t}),l=(a=r.text)==null?void 0:a.trim();if(l){const d=parseInt(l.trim(),10);if(Number.isInteger(d)&&d>=1&&d<=e.options.length)return{result:d}}}catch(r){throw console.error(`chooseHierarchyResolution_Service error (Attempt ${String(s+1)}/$${String(S+1)}):`,r),r}return{result:null}})},en=async(e,t,n,o,c,s)=>{if(!A())return console.error(`fetchCorrectedName_Service: API Key not configured. Cannot correct ${e} name.`),null;if(c.length===0)return console.warn(`fetchCorrectedName_Service: No valid names provided for ${e} to match against. Returning original: "${t}".`),t;const a=`The corrected ${e} name MUST be one of these exact, case-sensitive full names: [${c.map(d=>`"${d}"`).join(", ")}].`,r=`
...
If no suitable match can be confidently made, respond with an empty string.`,l=`Your task is to match a malformed ${e} name against a provided list of valid names, using narrative context. Respond ONLY with the best-matched string from the valid list, or an empty string if no confident match is found. Adhere to the theme context: ${s.systemInstructionModifier}`;return I(async d=>{var p;try{_(b.correction.icon);const{response:f}=await T({modelNames:[C,N,w],prompt:r,systemInstruction:l,temperature:E,label:"Corrections"}),i=((p=f.text)==null?void 0:p.trim())??null;if(i!==null){let h=i.trim();if(h=h.replace(/^['"]+|['"]+$/g,"").trim(),h==="")return console.warn(`fetchCorrectedName_Service (Attempt ${String(d+1)}/${String(S+1)}): AI indicated no match for ${e} "${t}" from the valid list.`),{result:null,retry:!1};if(c.includes(h))return console.warn("fetchCorrectedName_Service: Returned corrected Name ",h,"."),{result:h};console.warn(`fetchCorrectedName_Service (Attempt ${String(d+1)}/${String(S+1)}): AI returned name "${h}" for ${e} which is NOT in the validNamesList. Discarding result.`)}else console.warn(`fetchCorrectedName_Service (Attempt ${String(d+1)}/${String(S+1)}): AI call failed for ${e}. Received: null`)}catch(f){throw console.error(`fetchCorrectedName_Service error (Attempt ${String(d+1)}/${String(S+1)}):`,f),f}return{result:null}})},tn=async(e,t,n)=>{if(!A())return console.error("assignSpecificNamesToDuplicateNodes_Service: API Key not configured."),[];const o=new Map;e.forEach(s=>{const a=s.placeName.toLowerCase(),r=o.get(a)??[];r.push(s),o.set(a,r)});const c=[];for(const s of Array.from(o.values()))if(!(s.length<=1))for(let a=1;a<s.length;a+=1){const r=s[a],l=`You are an AI assistant disambiguating map location names in a text adventure game.
Theme: "${t.name}"
...
Description: ${r.data.description}`,d="Respond ONLY with a short Title Case name that distinguishes this location.",p=await I(async f=>{var i;try{_(b.correction.icon);const{response:h}=await T({modelNames:[C,N,w],prompt:l,systemInstruction:d,temperature:E,label:"Corrections",debugLog:n}),u=(i=h.text)==null?void 0:i.trim();if(u){const m=u.replace(/^['"]+|['"]+$/g,"").trim();if(m)return{result:m}}}catch(h){throw console.error(`assignSpecificNamesToDuplicateNodes_Service error (Attempt ${String(f+1)}/${String(S+1)}):`,h),h}return{result:null}});p&&c.push({nodeId:r.id,newName:p})}return c},We=(e,t)=>{const n=D(e),o=P(n);try{if(!o)throw new Error("JSON parse failed");if(!Array.isArray(o.npcResponses)||!o.npcResponses.every(s=>typeof s.speaker=="string"&&typeof s.line=="string")||!Array.isArray(o.playerOptions)||!o.playerOptions.every(s=>typeof s=="string")||o.dialogueEnds!==void 0&&typeof o.dialogueEnds!="boolean"||o.updatedParticipants!==void 0&&(!Array.isArray(o.updatedParticipants)||!o.updatedParticipants.every(s=>typeof s=="string")))return console.warn("Parsed dialogue JSON does not match DialogueAIResponse structure:",o),null;o.playerOptions.length===0&&(o.playerOptions=["End Conversation."]);const c=o;return t&&t.length>0&&c.npcResponses.forEach((s,a)=>{t[a]&&(s.thought=t[a])}),c}catch(c){return console.warn("Failed to parse dialogue JSON response from AI:",c),console.debug("Original dialogue response text:",e),null}},qe=(e,t)=>We(e,t),nn=async(e,t,n,o,c,s,a,r)=>{if(!A())return console.error("fetchCorrectedDialogueSetup_Service: API Key not configured."),null;const l=o.length>0?Ne(o," - "):"None specifically known in this theme yet.",d=ne(c,!0),p=s.map(u=>u.name).join(", ")||"Empty",i=`
...
Respond ONLY with the single, complete, corrected JSON object for 'dialogueSetup'.`,h="Correct a malformed 'dialogueSetup' JSON payload. Ensure 'participants' are valid NPCs, 'initialNpcResponses' are logical, and 'initialPlayerOptions' are varied with an exit option. Adhere strictly to the JSON format.";return I(async u=>{try{_(b.correction.icon);const{response:m}=await T({modelNames:[N,w],prompt:i,systemInstruction:h,responseMimeType:"application/json",temperature:E,label:"Corrections"}),y=P(D(m.text??""));if(y&&Je(y))return{result:y};console.warn(`fetchCorrectedDialogueSetup_Service (Attempt ${String(u+1)}/${String(S+1)}): Corrected dialogueSetup payload invalid. Response:`,y)}catch(m){throw console.error(`fetchCorrectedDialogueSetup_Service error (Attempt ${String(u+1)}/${String(S+1)}):`,m),m}return{result:null}})},rn=async(e,t,n,o)=>{if(!A())return console.error("fetchCorrectedDialogueTurn_Service: API Key not configured."),null;const c=t.map(r=>`"${r}"`).join(", ")||"None",s=`Role: You fix malformed JSON for a dialogue turn in a text adventure game.
...
Respond ONLY with the corrected JSON object.`,a=`Correct a malformed dialogue turn JSON object without altering the dialogue text. Speaker names must be among: ${c}. Adhere strictly to JSON format.`;return I(async r=>{var l;try{_(b.correction.icon);const{response:d}=await T({modelNames:[C,N,w],prompt:s,systemInstruction:a,temperature:E,label:"Corrections"}),p=((l=d.text)==null?void 0:l.trim())??null;if(p){const f=qe(p,o);if(f!=null&&f.npcResponses.every(i=>t.includes(i.speaker)))return{result:f};console.warn(`fetchCorrectedDialogueTurn_Service (Attempt ${String(r+1)}/${String(S+1)}): corrected response invalid or speakers not in list.`,p)}else console.warn(`fetchCorrectedDialogueTurn_Service (Attempt ${String(r+1)}/${String(S+1)}): AI returned empty response.`)}catch(d){throw console.error(`fetchCorrectedDialogueTurn_Service error (Attempt ${String(r+1)}/${String(S+1)}):`,d),d}return{result:null}})},te=(e,t)=>{switch(t){case"create":{const n=e;if((typeof n.holderId!="string"||n.holderId.trim()==="")&&(n.holderId=ie),Z(n,"create")){const o=n;return o.knownUses=H(o.knownUses),{action:t,item:n}}return null}case"change":{const n=e,o=typeof n.type=="string"?n.type:void 0,c=typeof n.status=="string"?n.status:void 0,s=o?R(o):null,a=c?c.toLowerCase():null,r=o?o.toLowerCase():null;if(s&&j.has(s)||r&&j.has(r)||a&&j.has(a)){const l={id:typeof n.id=="string"?n.id:void 0,name:typeof n.name=="string"?n.name:void 0};return ee(l)?{action:"destroy",item:l}:null}if(Z(e,"change")){const l=e;return l.knownUses=H(l.knownUses),{action:"change",item:e}}return null}case"addDetails":return he(e)?{action:"addDetails",item:e}:{action:"addDetails",item:e,invalidPayload:e};case"destroy":return ee(e)?{action:"destroy",item:e}:null;case"move":{const n=e;if(typeof n.id=="string"&&typeof n.newHolderId=="string"){const o={id:n.id,name:n.name,newHolderId:n.newHolderId};return{action:t,item:o}}return null}default:return null}},Xe=e=>{const t=D(e),n=P(t);if(!n)return null;let o=null;const c=(a,r)=>{const l=[];for(const d of a){if(!d||typeof d!="object")continue;const p=te(d,r);p&&l.push(p)}return l},s=a=>{const r=[];for(const l of a){if(!l||typeof l!="object")continue;const d=l,p=typeof d.action=="string"?d.action:void 0,f=d.item&&typeof d.item=="object"?d.item:void 0;if(p&&f){const i=te(f,p);i&&r.push(i)}}return r};if(Array.isArray(n))o={itemChanges:c(n,"create")};else if(typeof n=="object"){const a=n,r=[];Array.isArray(a.create)&&r.push(...c(a.create,"create")),Array.isArray(a.change)&&r.push(...c(a.change,"change")),Array.isArray(a.move)&&r.push(...c(a.move,"move")),Array.isArray(a.destroy)&&r.push(...c(a.destroy,"destroy")),Array.isArray(a.addDetails)&&r.push(...c(a.addDetails,"addDetails")),Array.isArray(a.itemChanges)&&r.push(...s(a.itemChanges)),o={itemChanges:r,observations:typeof a.observations=="string"?a.observations:void 0,rationale:typeof a.rationale=="string"?a.rationale:void 0}}return o},an=async(e,t,n,o,c,s,a,r,l,d)=>{if(!A())return console.error("fetchCorrectedItemChangeArray_Service: API Key not configured."),null;const p=`You are an AI assistant fixing a malformed inventory update JSON payload for a text adventure game.
...
{ "action": (${Pe}), "item": { ... } }
Valid item types: ${Y}. Holder IDs can be "${ie}", "${a??"unknown"}", companion IDs, or nearby NPC IDs from the context. Respond ONLY with the corrected JSON array.`;return I(async i=>{try{_(b.correction.icon);const{response:h}=await T({modelNames:[N,w],prompt:p,systemInstruction:f,responseMimeType:"application/json",temperature:E,label:"Corrections"}),u=P(D(h.text??"")),m=u?Xe(JSON.stringify(u)):null,y=m?m.itemChanges:null;if(y)return{result:y};console.warn(`fetchCorrectedItemChangeArray_Service (Attempt ${String(i+1)}/${String(S+1)}): corrected payload invalid.`,u)}catch(h){throw console.error(`fetchCorrectedItemChangeArray_Service error (Attempt ${String(i+1)}/${String(S+1)}):`,h),h}return{result:null}})};export{Y as $,V as A,G as B,rt as C,ct as D,xe as E,J as F,N as G,Qt as H,qt as I,Ze as J,zt as K,b as L,C as M,Et as N,Zt as O,Xt as P,Wt as Q,tn as R,Lt as S,K as T,xt as U,U as V,F as W,Ut as X,ke as Y,Me as Z,ie as _,st as a,x as a0,ut as a1,Le as a2,$e as a3,en as a4,Je as a5,Ht as a6,Ft as a7,nn as a8,Yt as a9,Te as aA,Gt as aB,Vt as aC,tt as aD,B as aE,W as aF,Tt as aG,ue as aH,mt as aI,it as aJ,ot as aK,at as aL,ft as aM,E as aa,Rt as ab,Mt as ac,qe as ad,rn as ae,kt as af,Ot as ag,Pe as ah,je as ai,Xe as aj,an as ak,Bt as al,Jt as am,M as an,St as ao,vt as ap,et as aq,yt as ar,ht as as,pt as at,gt as au,Ee as av,_e as aw,Ie as ax,nt as ay,jt as az,lt as b,dt as c,bt as d,Dt as e,Pt as f,Ct as g,$t as h,w as i,S as j,_t as k,Oe as l,Kt as m,Ge as n,Ve as o,Ye as p,ce as q,le as r,de as s,pe as t,A as u,T as v,It as w,Nt as x,wt as y,At as z};
