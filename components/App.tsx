...
const App: React.FC = () => {
  ...
  const {
    ...
    inventory, gameLog, isLoading, error, lastActionLog, themeHistory, mapData, // mapData from useGameLogic
    currentMapNodeId, mapLayoutConfig,
    isLoadingAfterRealityShift, isSpinnerShaking, isPostRealityShiftDisorientation,
    allCharacters, // allPlaces removed
    ...
    completeManualShiftWithSelectedTheme, // Added
    cancelManualShiftThemeSelection,    // Added
    isAwaitingManualShiftThemeSelection, // Added
    startCustomGame, 
    ...
    loadingReason,
    handleUndoTurn, // Added for DebugView
  } = gameLogic;
  ...
  const [isManualShiftThemeSelectionVisible, setIsManualShiftThemeSelectionVisible] = useState(false); // Added
  ...
  useEffect(() => {
    ...
    currentTheme, currentScene, actionOptions, mainQuest, currentObjective, // currentTheme is now the full object
    inventory, gameLog, lastActionLog, themeHistory, mapData, currentMapNodeId, mapLayoutConfig, 
    isPostRealityShiftDisorientation, allCharacters, score, 
  ...
  const handleSaveToFile = useCallback(() => {
    if (isLoading || !currentTheme || !!dialogueState) { // currentTheme is now the object
  ...
  const handleFileSelected = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (loadedFullState) {
        await applyLoadedGameState(loadedFullState);
        setUserRequestedTitleMenuOpen(false);
        setIsCustomGameSetupVisible(false); // Close custom setup if open
        setIsManualShiftThemeSelectionVisible(false); // Close manual shift theme selection
  ...
  const canPerformFreeAction = score >= FREE_FORM_ACTION_COST && !isLoading && !isPostRealityShiftDisorientation && hasGameBeenInitialized && !dialogueState;
  ...
  // Confirm manual reality shift (normal or custom)
  const confirmShift = () => {
    executeManualRealityShift(); // This now handles custom mode internally by setting a flag
    setShiftConfirmOpen(false);
  };

  // Effect to open theme selection modal for custom manual shift
  useEffect(() => {
  ...
  // Handlers for manual shift theme selection modal
  ...
  const closeSettings = () => {
    ...
    // Only return to title menu if it was the one that opened settings initially
    // Or if no game is active (title menu is forced open then)
    if (userRequestedTitleMenuOpen || !hasGameBeenInitialized) {
      setUserRequestedTitleMenuOpen(true);
    }
  };

  // Custom Game Setup Handlers
    ...
    <>
      <div className="min-h-screen bg-slate-900 text-slate-200 p-4 md:p-8 flex flex-col items-center">
        <header className="w-full max-w-screen-xl mb-6 text-center">
          ...
          {hasGameBeenInitialized && (
            <p className="text-slate-400 text-lg">An Adventure in Shifting Realities
              ...
              {currentTheme && !isPostRealityShiftDisorientation && <span className="block text-xs text-purple-400">Current Theme: {currentTheme.name}</span>}
              {isPostRealityShiftDisorientation && <span className="block text-xs text-red-400">Current State: Reality Unstable!</span>}
            </p>
          )}
        </header>

        {error && !isLoading && !isPostRealityShiftDisorientation && !dialogueState && hasGameBeenInitialized && (
        ...
        <main className={`w-full max-w-screen-xl grid grid-cols-1 lg:grid-cols-4 gap-6 flex-grow ${(isAnyModalOrDialogueActive) ? 'filter blur-sm pointer-events-none' : ''}`}>
          <div className="lg:col-span-2 space-y-6">
            {hasGameBeenInitialized && (
              <MainToolbar
                ...
                isPostRealityShiftDisorientation={isPostRealityShiftDisorientation}
            ...
            {error && isPostRealityShiftDisorientation && hasGameBeenInitialized && (
          ...
          <div className="lg:col-span-2 space-y-6 flex flex-col">
            <InventoryDisplay
              ...
              disabled={isLoading || isPostRealityShiftDisorientation || !!dialogueState || effectiveIsTitleMenuOpen || isCustomGameSetupVisible || isManualShiftThemeSelectionVisible } 
            />
          </div>
        </main>
        ...
        <ItemChangeAnimator
          ...
          isGameBusy={isAnyModalOrDialogueActive || isLoading || isPostRealityShiftDisorientation} 
        />
      ...
      <DebugView
        ...
        onUndoTurn={handleUndoTurn} // Pass the undo handler
      />
      ...
      />
      {/* Reusing CustomGameSetupScreen for Manual Shift Theme Selection */}
      <CustomGameSetupScreen
      ...
      {hasGameBeenInitialized && currentTheme && ( // Ensure currentTheme is not null
        <>
          <ImageVisualizer
            currentSceneDescription={currentScene}
            currentTheme={currentTheme} // Pass the full theme object
            mapData={mapData.nodes}
            allCharacters={allCharacters}
            localTime={localTime}
            localEnvironment={localEnvironment}
          ...
          <ConfirmationDialog
            ...
            isCustomModeShift={isCustomGameMode} // Pass custom mode status
          />
        </>
      )}
    </>
